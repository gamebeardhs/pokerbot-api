#!/usr/bin/env python3
"""
Continue TexasSolver Import: Resume from where we left off to complete the full 50K import
"""
...
def check_current_status():
    ...
    last_id = 500000  # Default starting point
    if result:
        last_id = int(result[0].replace('simple_', '')) + 1
    
    conn.close()
    
    return total_count, simple_count, last_id

def start_continuous_import():
    """Start the import process to run until completion."""
    
    total_count, simple_count, last_id = check_current_status()
    target = 50000  # Full target
    remaining = target - simple_count
    
    print(f"🚀 CONTINUING TEXASSOLVER IMPORT")
    print("=" * 33)
    print(f"Total database scenarios: {total_count:,}")
    print(f"Simple authentic scenarios: {simple_count:,}")
    print(f"Target: {target:,} scenarios")
    print(f"Remaining to import: {remaining:,}")
    print(f"Will continue from ID: {last_id}")
    print(f"Estimated time: {remaining/22/60:.1f} minutes at 22/sec")
    
    # Update the importer to continue from the right place
    updated_code = f'''#!/usr/bin/env python3
"""
Continue TexasSolver Import: Resume from exact position
"""
...
class ContinueAuthenticImporter:
    """Continue TexasSolver import from exact position."""
    
    def __init__(self, start_id={last_id}, start_count={simple_count}):
        self.db_path = "gto_database.db"
        self.scenarios_added = start_count  # Start from current count
    ...
    def run_continuous_import(self, target: int = 50000) -> None:
        """Run continuous import until target reached."""
        
        print(f"🚀 CONTINUING AUTHENTIC TEXASSOLVER IMPORT")
        print("=" * 39)
        print(f"Target: {{target:,}} total authentic scenarios")
        print(f"Starting from: {{self.scenarios_added:,}} scenarios")
        print(f"Remaining: {{target - self.scenarios_added:,}} scenarios")
        print("Will run continuously until complete...")
        ...
        while self.scenarios_added < target:
            ...
            rate = (self.scenarios_added - {simple_count}) / elapsed if elapsed > 0 else 0
        ...
        elapsed = time.time() - self.start_time
        final_rate = (self.scenarios_added - {simple_count}) / elapsed if elapsed > 0 else 0
        print(f"\\n🎉 CONTINUOUS IMPORT COMPLETE")
        print(f"Successfully imported: {{self.scenarios_added:,}} total authentic scenarios")
        print(f"Session imported: {{self.scenarios_added - {simple_count}:,}} new scenarios")
        print(f"Session time: {{elapsed:.1f}}s at {{final_rate:.1f}} scenarios/second")

if __name__ == "__main__":
    importer = ContinueAuthenticImporter()
    importer.run_continuous_import(50000)
'''
    
    # Write the updated importer
    with open("continue_texassolver_import.py", "w") as f:
        f.write(updated_code)
    
    print(f"Starting continuous import process...")
    return True

if __name__ == "__main__":
    start_continuous_import()